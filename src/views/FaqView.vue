...
<template>
  <ion-page>
    <ion-content :fullscreen="false">
      <main>
        <TopBar title="FAQ" :backLink="$router.options.history.state.back.substring(0, 4) != '/faq'
        ...
        <div v-if="apiDataStore.faq.ready" class="padding-main">
          <RouterLink :to="'/faq/' + data.id" v-for="(data, index) in apiDataStore.faq.data" :key="index">
            <ItemBox :bigText="data.question" :smallText="data.answer.length > 40 + 3 ? data.answer.substring(0, 40) + '...' : data.answer
